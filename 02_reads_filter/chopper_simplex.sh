#!/bin/bash
#SBATCH --job-name=chopper_simplex.sh
#SBATCH -o chopper_simplex.out
#SBATCH -e chopper_simplex.err
...
INPUT_FASTQ=/nfs/scratch/sanghu/johndory/chap2_ref_genome/02_reads_filter/duplex_called/chopper_fastq/nextflow/simplex/clean/merged_simplex.porechop.nextflow.fastq.gz
...
#gunzip -c $INPUT_DIR/merged_simplex_porechop.fastq.gz | chopper -q 11 -t 50 | gzip > $OUT_DIR/simplex.p.nextflow.chopper_q11.fastq.gz
#gunzip -c $INPUT_DIR/merged_simplex_porechop.fastq.gz | chopper -q 11 --headcrop 50 -t 50 | gzip > $OUT_DIR/simplex.p.nextflow.chopper_q11_h50.fastq.gz
#gunzip -c $INPUT_FASTQ | chopper -q 8 --headcrop 50 -t 50 | gzip > $OUT_DIR/simplex.porechop.nextflow.chopper_q8_h50.fastq.gz
#gunzip -c $INPUT_FASTQ | chopper --headcrop 50 -t 50 | gzip > $OUT_DIR/simplex.porechop.nextflow.chopper_h50.fastq.gz
gunzip -c $INPUT_FASTQ | chopper --tailcrop 50 -t 50 | gzip > $OUT_DIR/simplex.porechop.nextflow.chopper_t50.fastq.gz
#gunzip -c $INPUT_FASTQ | chopper -q 8 -t 50 | gzip > $OUT_DIR/simplex.porechop.nextflow.chopper_q8.fastq.gz
